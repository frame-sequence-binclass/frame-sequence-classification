...
def save_checkpoint(state, filename="./cnn3d/cnn3d_checkpoint.pth.tar"):
...
def load_trained_model(image_size: int, sequence_length: int, pretrained: bool):
    if pretrained:
        model = Binary3DCNN(model_type='pretrained', sequence_length=sequence_length, img_size=image_size)
    else:
        model = Binary3DCNN(model_type='scratch', sequence_length=sequence_length, img_size=image_size)
    checkpoint = torch.load("./cnn3d/cnn3d_best_model.pt", weights_only=True)
...
def train_model(train_loader, val_loader, image_size, sequence_length, pretrained):
    ...
    start_epoch = 0
    best_val_auc = 0.0
    checkpoint_file = "./cnn3d/cnn3d_checkpoint.pth.tar"
    ...
    num_epochs = 200
    save_every = 3
    early_stopping = EarlyStopping(patience=20, min_delta=1e-4, verbose=True, mode='max')
    ...
    for epoch in range(start_epoch, num_epochs):
        if pretrained:
            ...
            for layer_name, target_epoch, new_lr in unfreeze_plan:
                if epoch == target_epoch:
                    ...
                    layer = getattr(model.base_model, layer_name)
        ...
        if val_auc >= best_val_auc:
            ...
            torch.save(model.state_dict(), "./cnn3d/cnn3d_best_model.pt")
